...
function generateIndex(dir) {
  ...
  const content = `
    ...
    .join("")}</ul></div><script>function filterFiles() {
      ...
      function getPreviousDirectoryPath(currentPath) {
    ...
    // Remove the last part (current directory)
    pathParts.pop();

    // Join the remaining parts to form the parent directory path
    const prevDirPath = pathParts.join('/') || '/'; // Ensure at least '/' if empty
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const prevDirPath = getPreviousDirectoryPath(currentDir);
    document.getElementById('prevDirLink').setAttribute('href', prevDirPath);
  });</script>
  `;
